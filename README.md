...
当前控制会话用 `6064` 实际位置初始化 CSP 的 `607A` 保持目标，尚未提供变化位置、速度或转矩命令接口。
...
供应商原始资料位于本地 `docs/lz-joint/`，由于尚无再分发授权，不提交到公开仓库。设备配置
引用的 ESI 存在时，项目校验会解析其中的身份、PDO、模块和同步事实；其他供应商资料不参与构建门控。
