        ...
        else
        {
            ...
            for (size_t axis = 0U; axis < session->plan->axis_count; ++axis)
            {
                ...
                if (wkc > 0)
                {
                    ...
                    /*
                     * 首周期的值原先在这里 fprintf 到 stderr。删掉：同样的字节刚刚
                     * 写进 axes[].rx_error_counter / forwarded_rx_error_counter /
                     * ecat_processing_unit_error_counter / pdi_error_*，报告里就
                     * 有——判据不该只活在日志里，而日志也不该重复报告已经有的东西。
                     * 抽样点从 exchange 1 开始（1 % 50 == 1），所以首周期必然被采到。
                     */
                }
                else
                {
