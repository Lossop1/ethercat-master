...
声明的收发字段必须实际存在于所选 PDO 方案。已安装受控 ESI 时，构建校验还会核对
`AssignActivate`、最小周期以及 SM2/SM3 同步类型默认值。

部署中的 `operation_profile_ids` 为空表示禁用过程数据会话；非空时必须为拓扑使用的每种设备配置
恰好启用一个已批准方案，且所有方案周期一致。会话启动命令不接受模式、周期或同步参数覆盖。

当前 Orange Pi 台架部署已引用经过负责人批准的 `DC+CSP+1 ms` 运行方案。该引用只赋予会话计划构建
资格，不会提高现有 PRE-OP 指纹工具的代码能力，也不代表已经允许进入 OP、使能驱动器或运动。
SM 文件仍为未批准草案，部署不会加载它。`emaster-cia-preflight` 是独立的更高状态验证流程，
即使得到操作者确认，也只发送禁止使能、零目标的首帧并在退出时恢复 INIT，不代表获得 CiA 402
状态转换或运动授权。
