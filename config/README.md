...
- `external_target_timeout_ms`：可选。外部控制器超过此时间未更新目标即退化为保持（HOLD），
  ...
  力矩模式（CST）下这个超时不是"保持"，而是把目标回零——"保持"在力矩里等于继续出力。
- `external_target_torque_limit_per_mille`：可选。外部目标的力矩上限，单位是额定力矩的
  千分比（CiA402 6071 相对 6076 的口径），1..1000 或 null（null = 不设上限）。只在力矩
  模式（CST）下有意义：位置模式有软限位和单步限幅，那两条的位置量纲阈值在力矩下一条也
  拦不住。超限与单步超限同判，整会话中止。
...
方案，一次只能运行一个；物理独占由运行时抢占网卡自然保证，不是静态可判定的冲突。校验器因此
不约束网口唯一，只约束 `deployment_id` 与 `run_report_path` 唯一 —— 后者防止不同场景的审计报告
互相覆盖，因为报告每次会话原子覆盖。`enp49s0` 只出现在 Orange Pi 部署实例和测试证据中，
通用代码与拓扑配置均不感知该名称。
