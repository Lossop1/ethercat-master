...
| P3.2 | 编码在发送之后 ⇒ 固有 1 周期延迟 | 明确记录为契约，或改为同周期编码 |
| P3.3 | 单位换算散落在工具层（`main.c:107` 用电机侧 counts，README 写负载侧） | 换算集中到 L4，L3 只进出 counts，两处口径统一 |
| P3.4 | demo 正弦违反 phase-1 禁令且幅值口径错 | demo 与生产路径分离 |
