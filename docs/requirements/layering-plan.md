...
| P2.5 | Fault Reset 调用路径已存在但未真机验证 | `Fault → Fault Reset → Switch On Disabled` 全程可走 | 已实现·未验证 |
...
P4.1 的 limit 取值必须在 P1.4 完成后实测确定，否则邮箱阻塞往返会直接吃掉周期预算
（见基线 2.3）。

**P4.1 调优结果**（2026-09-12，基于 P1.4 双从站数据）：

对比测试 limit=2/4/8 的 round_trip 百分位（3 分钟采样）：
```
         p50        p99        p99.9      p99.999
limit=2: 155~156   248~249    276~277    336~337 μs
limit=4: 155~156   250~251    279~280    332~333 μs
limit=8: 155~156   247~248    275~276    335~336 μs
```
...
| # | 问题 | 完成判据 | 状态 |
| --- | --- | --- | --- |
| P5.1 | 运行期模式切换未实现（模式在计划构建期固定） | 0x119800 方案下 OP 态切换 CSP/CSV/CST | 未做 |
| P5.2 | ~~外部目标路径硬要求 mode==8（`session_target.c:41-45`）~~ | 三模式均可接收对应目标 | 已验证 |
| P5.3 | `fixed_csv` 的 1A02 内容 ESI 与手册冲突，未验证 | 真机读回确认 | 硬件阻塞 |
...
**剩余硬件阻塞项**：P5.3（`fixed_csv` ESI 验证）、P5.4（3 轴及以上时序）、P2.5 的触发测试（需先解决驱动器静置不响应问题）。
