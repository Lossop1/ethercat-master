...
| P2.5 | Fault Reset 零调用者 | `Fault → Fault Reset → Switch On Disabled` 全程可走 | **已完成（代码实现）** |
| P2.6 | 运行期 603F 恒为 0 | 运行期读取（走 P4 的邮箱通道） | **已完成（P4.3 线程周期读取）** |
...
| P4.3 | 电流/电压/温度报告字段恒为 0（死代码） | 走 SDO 慢速通道填充，或删除字段 | **已完成（50ms 线程读取 6078h/603Fh）** |
| P4.4 | 错误计数器 SOEM 只清零不读回 | 应用侧 FPRD 读 0x0300-0x030F | **已完成** |
| P4.5 | SOEM 错误环无互斥（RT push / 非 RT pop） | 加同步或换无锁结构 | **已完成** |
...
| P5.2 | ~~外部目标路径硬要求 mode==8（`session_target.c:41-45`）~~ | ✅ 三模式均可接收对应目标（2026-09-11）|
